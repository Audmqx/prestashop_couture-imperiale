...
class BoxtalconnectAjaxModuleFrontController extends \ModuleFrontController
{
    ...
    {
        $ziptown = implode(' ', array_filter(array($parcelPoint->zipcode, $parcelPoint->city)));
        $address = implode(', ', array_filter(array($parcelPoint->address, $ziptown)));
        return $address;
    }
    ...
    public function getSelectedCarrierTextHandler($cartId, $selectedCarrierId)
    {
        ...
        if (ShippingMethodUtil::hasSelectedParcelPointNetworks($selectedCarrierId)) {
            ...
            if (false !== $pointsResponse) {
                ...
                $message = $boxtalConnect->l('Your parcel point:');
                $parcelPoint = ParcelPointUtil::getChosenPoint($cartId, $selectedCarrierCleanId);
                if (null === $parcelPoint) {
                    $message = $boxtalConnect->l('Closest parcel point:');
                    $parcelPoint = ParcelPointController::getClosestPoint($cartId, $selectedCarrierCleanId);
                    if (null === $parcelPoint) {
                ...
                if ($parcelPoint->distanceFromSearchLocation !== null) {
                    $distance = round($parcelPoint->distanceFromSearchLocation / 1000, 1);
                    $distanceFromSearchLocation = ' (' . sprintf($boxtalConnect->l('%s km away'), $distance) . ')';
                }

                $text .= '<br/><span class="bx-parcel-client">'
                    . '<b>' . $message . '</b>'
                    . '<span class="bx-parcel-name">' . $parcelPoint->name . ' </span></br>'
                    . '<span class="bx-parcel-address">'
                        . $this->getParcelPointOneLineAddress($parcelPoint)
                        . $distanceFromSearchLocation . '</span>'
                    . '</span>'
                    . '<br/><span class="bx-select-parcel">' . $boxtalConnect->l('Choose another') . '</span>';
            }
        }
