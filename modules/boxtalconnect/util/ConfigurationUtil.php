...
class ConfigurationUtil
{
    ...
    private static $ALL_CONFIGS = array(
        'BX_ACCESS_KEY',
        'BX_SECRET_KEY',
        'BX_MAP_BOOTSTRAP_URL',
        'BX_MAP_TOKEN_URL',
        'BX_MAP_LOGO_IMAGE_URL',
        'BX_MAP_LOGO_HREF_URL',
        'BX_PP_NETWORKS',
        'BX_PAIRING_UPDATE',
        'BX_ORDER_PREPARED',
        'BX_ORDER_SHIPPED',
        'BX_ORDER_DELIVERED',
        'BX_TRACKING_URL_PATTERN',
        'BX_HELP_CENTER_URL',
        'BX_SHIPPING_RULES_URL',
        'BX_TUTO_URL'
    );
    ...
    public static function getAll($shopGroupId = null, $shopId = null)
    {
        ...
        foreach (self::$ALL_CONFIGS as $config) {
            $all_configs[$config] = self::get($config, $shopGroupId, $shopId, null);
        }
        return $all_configs;
    }
     ...
     *
     * @return string shipping rules url
     */
    public static function getShippingRulesUrl()
    {
        $url = self::get('BX_SHIPPING_RULES_URL');
     ...
     * Get tuto url.
     *
     * @return string tuto url
     */
    public static function getTutoUrl()
    {
        $url = self::get('BX_TUTO_URL');
    ...
    {
        $url = self::get('BX_CONFIGURATION_URL');
    ...
    {
        self::set('BX_CONFIGURATION_URL', $url);
    }
    ...
    public static function deleteConfiguration()
    {
        ...
        self::deleteAllShops('BX_CONFIGURATION_URL');
    ...
    private static function parseLinksConfiguration($body)
    {
        ...
        if (is_object($body) && property_exists($body, 'shippingPreferencesUrl')) {
            self::set('BX_SHIPPING_RULES_URL', $body->shippingPreferencesUrl);
        }
        if (is_object($body) && property_exists($body, 'configurationKitlUrl')) {
            self::set('BX_TUTO_URL', $body->configurationKitlUrl);
        }
        return true;
    }
}
