...
class BoxtalConnect extends Module
{
    ...
    {
        $url = $this->getContext()->link->getAdminLink('AdminShippingMethod', false);

        if ($url !== null && $url !== '') {
            Boxtal\BoxtalConnectPrestashop\Util\ConfigurationUtil::setConfigurationUrl($url);
        }
    }
    ...
    public function addJs($tag, $pathFromModule, $priority)
    {
        ...
        $path = $this->_path . $pathFromModule;

        if (method_exists($controller, 'registerJavascript')) {
            $controller->registerJavascript($tag, $path, array('priority' => $priority, 'server' => 'local'));
        }
    ...
    public function addCss($tag, $pathFromModule, $priority)
    {
        ...
        if (method_exists($controller, 'registerStylesheet')) {
            $controller->registerStylesheet($tag, $path, array('priority' => $priority, 'server' => 'local'));
        }
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        if (Boxtal\BoxtalConnectPrestashop\Controllers\Misc\NoticeController::hasNotices()) {
            $this->addJs('bx-polyfills', '/views/js/polyfills.min.js', 99);
            $this->addJs('bx-notices', '/views/js/notices.min.js', 100);
            $this->addCss('bx-notices', '/views/css/notices.css', 100);
        }

        if ('AdminOrdersController' === get_class($controller) && false !== Tools::getValue('id_order')) {
            $this->addCss('bx-tracking', '/views/css/tracking.css', 100);
        }

        if ('AdminShippingMethodController' === get_class($controller)) {
            $this->addCss('bx-tracking', '/views/css/settings.css', 100);
        }
    }
