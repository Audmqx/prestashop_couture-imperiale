...
{*       <p class="h4 card-title">{l s='Product customization' d='Shop.Theme.Catalog'}</p>
      {l s='Don\'t forget to save your customization to be able to add to cart' d='Shop.Forms.Help'} *}
            <p class="h4 card-title">En cas de pré-commande:
      ...
      {block name='product_customization_form'}
        <form method="post" action="{$product.url}" enctype="multipart/form-data">
            ...
            {foreach from=$customizations.fields item="field"}
              <li class="product-customization-item">
                ...
                {if $field.type == 'text'}
                  ...
                  {if $field.text !== ''}
                      <h6 class="customization-message">Votre message pour la pré-commande:
          ...
          <div class="clearfix">
            <button class="btn btn-primary float-xs-right" type="submit" name="submitCustomizedData">Enregistrer votre taille</button>
          </div>
        </form>
      {/block}
